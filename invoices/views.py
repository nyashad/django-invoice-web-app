...
from invoices.models import Company, Customer, GlobalSettings, Invoice, MailInfo, OrderLine, Product
...
def make_invoice(request, id):
    ...
    if request.method == 'POST':
        ...
        order_formset = OrderLineFormSet(request.POST, instance=invoice, queryset=order_lines)
        ...
        if order_formset.is_valid():
            ...
            for order_form in order_formset:
                ...
                try:
                    product = Product.objects.get(pk=order_form.cleaned_data["product"].id)
        ...
        order_formset = OrderLineFormSet(instance=invoice, queryset=order_lines)
    context = {
        'invoice': invoice,
...
def save_invoice_pdf(request, inv_id):
    ...
    context = {
        ...
        'empty_rows': range(16 - len(order_lines)),
    }
    pdf_render = WeasyTemplateResponse(request=request, template='invoices/invoice_pdf.html', context=context).rendered_content
    pdf_file_name = "{}-{}_{}_{}.pdf".format(invoice.sequence, invoice.number, invoice.customer.company.name.replace(" ", "-"), invoice.issued_date)
...
def send_invoices(request):
    if request.method == 'POST':
        ...
        invoices_list = Invoice.objects.filter(id__in=invoices_list).order_by('customer')
        global_settings = GlobalSettings.objects.filter().first()
        customers = Customer.objects.all()
        invoices_queryset = None
        for customer in customers:
            invoices_queryset = Invoice.objects.filter(id__in=invoices_list).filter(customer=customer).values_list('sequence', 'number', 'pdf_document', 'mail_info')
            if not invoices_queryset:
                continue
            if send_invoice_email(GMAIL, GMAIL_PASSWORD, global_settings.issuer.company.name, customer.company.contact.email, customer.company.contact.name, customer.company.contact.cc_email, invoices_queryset):
                for invoice in invoices_queryset:
                    mail_info_id = invoice[3]
                    MailInfo.objects.filter(pk=mail_info_id).update(status="Delivered", sent_timestamp=datetime.now())
            else:
                for invoice in invoices_queryset:
                    mail_info_id = invoice[3]
                    MailInfo.objects.filter(pk=mail_info_id).update(status="Failed")
    ...
    if request.method == 'POST':
        ...
        if contact_form.is_valid() and address_form.is_valid() and company_form.is_valid():
            if customer.company:  # Update existing company, contact, and address
                ...
                company.customer_information_file_number = company_form.cleaned_data['customer_information_file_number']
                ...
                company = Company.objects.create(name=company_form.cleaned_data['name'], customer_information_file_number=company_form.cleaned_data['customer_information_file_number'], contact=contact, address=address)
                customer.company = company
                customer.save()
            messages.success(request, 'Customer saved successfully')
            return redirect('view_customers')
    else:  # GET request
        contact_form = ContactForm(instance=customer.company.contact) if customer.company else ContactForm()
        address_form = AddressForm(instance=customer.company.address) if customer.company else AddressForm()
        company_form = CompanyForm(instance=customer.company) if customer.company else CompanyForm()
    context = {
...
def save_all_invoices_pdf(request):
    ...
    for invoice in invoices:
        MailInfo.objects.filter(pk=invoice.mail_info_id).update(status="Delivered", sent_timestamp=datetime.now())
